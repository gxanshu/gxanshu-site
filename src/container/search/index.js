...
function OptionText({ searchWords, textToHighlight }) {
  ...
  const highlightedText = chunks.map((chunk) => {
    ...
    if (highlight) {
      return (
        <Box as="mark" bg="transparent" color="blue.500">
          {text}
        </Box>
      );
    } else {
...
function OmniSearch() {
  ...
  const client = algoliasearch(
    "KBJO3GFGBN",
    "7fbd454a16c06edb277a0842b6b48096"
  );
  ...
  async function Query(e) {
    ...
    if (e.target.value.length < 2) {
      ...
      const search_client = client.initIndex("doc");
      search_client.search(e.target.value).then(({ hits }) => {
        setResults(hits);
      });
    }
  }
